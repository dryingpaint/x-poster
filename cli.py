...
def start():
    ...
    try:
        result = subprocess.run(["npx", "supabase", "start"], check=True)
...
def stop():
    ...
    try:
        result = subprocess.run(["npx", "supabase", "stop"], check=True)
...
def reset():
    ...
    try:
        result = subprocess.run(["npx", "supabase", "db", "reset"], check=True)
...
def create_migration(name: str):
    ...
    try:
        result = subprocess.run(["npx", "supabase", "migration", "new", name], check=True)
        console.print("[green]✅ Migration file created successfully![/green]")
        console.print(
            "[yellow]💡 Edit the migration file in supabase/migrations/ before applying![/yellow]"
        )
...
def diff(name: str):
    ...
    try:
        result = subprocess.run(["npx", "supabase", "db", "diff", "-f", name], check=True)
...
def link(project_ref: str):
    ...
    try:
        result = subprocess.run(
            ["npx", "supabase", "link", "--project-ref", project_ref], check=True
        )
